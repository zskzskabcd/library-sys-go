...
type Reader struct {
	Model
	Name      string `json:"name" gorm:"type:varchar(100);not null;index" binding:"required" example:"张三"`                 // 用户名
	Gender    string `json:"gender" gorm:"type:varchar(6);not null" binding:"required" example:"男"`                        // 性别
	Phone     string `json:"phone" gorm:"type:varchar(20);not null;uniqueIndex" binding:"required" example:"18888888888"`  // 手机号码
	StudentNo uint   `json:"studentNo" gorm:"type:int;not null;uniqueIndex" binding:"required,min=0" example:"2018000000"` // 学号
